...
### UHF amateur DMR simplex

| Channel | Freq (MHz) | Mode |
|---|---|---|
| DMR UHF Calling | 446.000 simplex | DMR CC1 TS1 |
| DMR UHF 446.025 | 446.025 simplex | DMR CC1 TS1 |
| DMR UHF 446.050 | 446.050 simplex | DMR CC1 TS1 |
| DMR UHF 446.075 | 446.075 simplex | DMR CC1 TS1 |
| DMR UHF 446.100 | 446.100 simplex | DMR CC1 TS1 |

These channels use the frequencies of SSRF-Lite's U.S. amateur UHF simplex
plan with DMR color code 1 and timeslot 1.
...
The DCS-toned ChiOff channels use DCS 023, matching the current shared
community-net configuration. `ChiO ROAD` instead uses CTCSS 141.3.

### DCS polarity

Every DCS-toned channel is **normal (`N`)** polarity, stated explicitly via
...
- All stable IDs include the `chio` namespace after their schema type prefix
  (for example, `org_chio`, `stn_chio_gmrs`, and `chain_chio_all`) so
  downstream generators can distinguish shared community records from the
  authoritative library and personal overlays.
